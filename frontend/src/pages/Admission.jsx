...
export const Admission = () => {
  ...
  const currentFee = feeStructure[selectedFeeGradeIndex] || feeStructure[0];
  const annualTuitionTotal = currentFee.tuitionFeePerTerm * currentFee.termsPerYear;
  const grandTotalEstimate = annualTuitionTotal + currentFee.annualCharges + currentFee.developmentFee + (includeTransport ? busFee : 0);
 ...
 <span className="text-xs text-slate-500 font-inter mt-0.5 block">GPS AC fleet coverage (+₹{busFee.toLocaleString()}/yr)</span>
 </div>
 <button
 type="button"
 ...
 <span className="font-bold">₹{busFee.toLocaleString()}</span>
 </div>
 )}
 </div>
