        ...
        <div className="bg-slate-800/90 border border-slate-700/80 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg backdrop-blur-md">
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full bg-slate-900/80 border border-slate-700 rounded-xl py-2.5 pl-10 pr-4 text-xs .text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-4 py-2 text-xs rounded-xl flex items-center gap-2 cursor-pointer shadow-md transition-all shrink-0"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredItems.map((item) => (
            <motion.div
              ...
              className="bg-slate-800/90 border border-slate-700/80 rounded-3xl overflow-hidden shadow-xl flex flex-col justify-between group hover:border-teal-500/60 transition-all"
            >
              <div>
                <div className="h-48 bg-slate-900 relative overflow-hidden">
                  ...
                  <span className="absolute top-3 left-3 bg-black/70 text-teal-300 border border-teal-500/40 text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider backdrop-blur-xs">
                    {item.category}
                  </span>
                </div>

                <div className="p-5 space-y-2">
                  <h3 className="font-bold text-white text-base leading-snug font-poppins">
                    {item.title}
                  </h3>
                  {item.caption && (
              ...
              {/* Actions Footer */}
              <div className="p-4 bg-slate-900/40 border-t border-slate-700/60 flex items-center justify-between">
                <span className="text-[10px] text-slate-500 font-mono">ID: {item.id}</span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleOpenEditModal(item)}
                    className="px-3 py-1.5 bg-slate-700 text-slate-300 hover:text-white rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">Image / Media URL *</label>
                    <input
                      type="text"
                      required
                      value={formData.url}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"
                    />
                  </div>
