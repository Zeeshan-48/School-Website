        ...
        <div className="bg-slate-800/90 border border-slate-700/80 rounded-3xl overflow-hidden shadow-xl">
          {filteredApplicants.length === 0 ? (
            <div className="p-12 text-center space-y-3">
              ...
              <p className="text-slate-400 text-sm font-semibold">No job applicants found matching your search criteria.</p>
            </div>
          ) : (
                  ...
                  {filteredApplicants.map((app) => (
                    <tr key={app.id} className="hover:bg-slate-750/60 transition-colors">
                      <td className="p-4">
                        ...
                        >
                          <span className="font-extrabold text-sm text-white block hover:text-amber-400 transition-colors">
                            {app.fullName}
                          </span>
                      ...
                      <td className="p-4">
                        <span className="font-bold text-amber-300 bg-amber-950/80 border border-amber-800 px-2.5 py-1 rounded-lg">
                          {app.jobTitle}
                        </span>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <select
                          ...
                          className={`text-[11px] font-bold px-2.5 py-1 rounded-xl border focus:outline-none cursor-pointer ${app.status === 'Shortlisted' ? 'bg-emerald-950 text-emerald-300 border-emerald-800' :
                              app.status === 'Under Review' ? 'bg-blue-950 text-blue-300 border-blue-800' :
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
              ...
              >
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-amber-400 bg-amber-950 px-3 py-1 rounded-full border border-amber-800">
                ...
                <div className="grid grid-cols-2 gap-3 bg-slate-900/80 rounded-2xl p-4 border border-slate-700/60 text-xs">
                  <div>
                    <span className="text-slate-400 block">Applied Role:</span>
                    <span className="text-white font-bold text-sm">{selectedApplicant.jobTitle}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block">Experience:</span>
                    <span className="text-white font-bold">{selectedApplicant.experience}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-slate-400 block">Attached Resume:</span>
                  ...
                  <div>
                    ...
                    <span className="text-slate-300 font-mono">{new Date(selectedApplicant.appliedAt).toLocaleString()}</span>
                  </div>
                </div>

                {selectedApplicant.coverLetter && (
                  <div>
                    <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Cover Statement</h4>
                    <div className="bg-slate-900/60 rounded-2xl p-4 border border-slate-700/60 text-xs text-slate-200 leading-relaxed whitespace-pre-wrap">
                      {selectedApplicant.coverLetter}
                    </div>
                  </div>
                )}
          ...
          {deleteConfirmId && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs">
              ...
              >
                ...
                <h3 className="text-lg font-bold text-white">Delete Applicant?</h3>
